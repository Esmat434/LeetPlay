...
from .permissions import (
    IsAuthenticationToken,AllowAnyUser
)
...
class LogoutView(APIView):
    ...
    def delete(self,request):
        try:
            ...
            token.delete()
            logout(request)
            return Response({"success":"You successfully loged out."},status=status.HTTP_200_OK)
...
class PasswrdForgotView(APIView):
    ...
    def post(self,request):
        ...
        if serializer.is_valid():
            return Response({'success':"Your email verified successfully and send change password token to your email."},status=status.HTTP_200_OK)
        return Response(serializer.errors,status=status.HTTP_400_BAD_REQUEST)
...
class SetNewPasswordView(APIView):
    ...
    def post(self,request,token):
        ...
        if not token:
            return Response({"error":"Please create another token this token is expired."},status=status.HTTP_400_BAD_REQUEST)
        ...
        if serializer.is_valid():     
            ...
            token.user.save()
            token.delete()
            return Response({"success":"Your password changed successfully. Please login to your account."},status=status.HTTP_200_OK)
        return Response(serializer.errors,status=status.HTTP_400_BAD_REQUEST)
