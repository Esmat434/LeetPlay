...
class TestSignupView(TestCase):
    ...
    @patch(
        "django_recaptcha.fields.ReCaptchaField.clean",
        return_value="dummy-recaptcha-response",
    )
...
class TestLoginView(TestCase):
    def setUp(self):
        ...
        self.user = CustomUser.objects.create_user(
            username="test",
            email="testemal@gmail.com",
            birth_date="2000-03-4",
            password="Test111@",
        )
        self.data = {"username": "test", "password": "Test111@", "remember_me": True}
    ...
    def test_login_view_post_method_invalid_password(self):
        data = {"username": "test", "password": "dskjljkjdsl", "remember_me": True}
    ...
    def test_login_view_post_method_invalid_authentication(self):
        data = {"username": "testuser", "password": "TestUser123@", "remember_me": True}
        response = self.client.post(reverse("login"), data)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "accounts/login.html")
        self.assertIsInstance(response.context["form"], LoginForm)
