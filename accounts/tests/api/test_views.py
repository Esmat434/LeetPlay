...
from accounts.api.serializers import (
    UserSerializer,UserUpdateSerializer,LoginSerializer,CheckEmailExistsSerializer,
...
class TestCreateUserView(APITestCase):
    ...
    def test_create_user_view_validate_data(self):
        data = {
            ...
            'password':'Test111@',"password2":"Test111@"
        }
    ...
    def test_create_user_view_invalid_birth_date(self):
        data = {
            'username':'test','email':'test@gmail.com','birth_date':'2024-02-03',
            'password':'Test111@','password2':'Test111@'
        }
        response = self.client.post(reverse('api-signup'),data=data)
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
        self.assertEqual(str(response.data['birth_date'][0]),'your age must be at least 18+.')
    ...
    def test_create_user_view_invalid_password_validation(self):
        ...
        response = self.client.post(reverse('api-signup'),data=data)
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
        self.assertEqual(str(response.data['non_field_errors'][0]),'password must be 8 charecter or greather.')

    def test_create_user_view_invalid_password_match(self):
        data = {
            'username':'test','email':'test@gmail.com','birth_date':'2000-03-01',
            'password':'Test111@','password2':'Mistake111@'
        }    
        response = self.client.post(reverse('api-signup'),data=data)
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
    ...
    def test_create_user_view_invalid_data(self):
        data = {
            'username':'','email':'','birth_date':'','password':'','password2':''
        }
        response = self.client.post(reverse('api-signup'),data=data)
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
        ...
        self.assertEqual(str(response.data['birth_date'][0]),'Date has wrong format. Use one of these formats instead: YYYY-MM-DD.')
...
class TestGetUpdateUserView(APITestCase):
    ...
    def test_update_user_view_validate_data(self):
        data = {
            'username':'change@gmail.com','email':'change@gmail.com'
        }
    ...
    def test_update_user_view_invalid_birth_date(self):
        ...
        response = self.client.put(reverse('api-user-update'),data=data)
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
        self.assertEqual(str(response.data['birth_date'][0]),'You must be at least 18+.')
...
class TestLoginView(APITestCase):
    ...
    def test_login_view_invalid_password(self):
        ...
        response = self.client.post(reverse('api-login'),data=data)
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
        self.assertEqual(str(response.data['password'][0]),'password must be 8 charecter or greather.')
...
class TestPasswordForgotView(APITestCase):
    ...
    def test_password_forgot_view_validate_data(self):
        ...
        self.assertEqual(data['success'],"Your email verified successfully and send change password token to your email.")
...
class TestSetNewPasswordView(APITestCase):
    ...
    def test_set_new_password_view_validate_data(self):
        ...
        response =  self.client.post(reverse("api-set-new-password",args=[self.key.token]),data=data)
        self.assertEqual(response.status_code,status.HTTP_200_OK)
        data = json.loads(response.content)
        self.assertEqual(data['success'],"Your password changed successfully. Please login to your account.")
    ...
    def test_set_new_password_view_invalid_token(self):
        ...
        response = self.client.post(reverse("api-set-new-password",args=[uuid.uuid4()]),data=data)
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
        data = json.loads(response.content)
        self.assertEqual(data['error'],"Please create another token this token is expired.")